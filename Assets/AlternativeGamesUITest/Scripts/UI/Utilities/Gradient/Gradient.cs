...
public class Gradient : MonoBehaviour, IMaterialModifier
{
    ...
    public Material GetModifiedMaterial(Material baseMaterial)
    {
        ...
        if (_materialInstance == null || _materialInstance.shader.name != "UI/GradientMaskedElement")
        {
            var shader = Shader.Find("UI/GradientMaskedElement");
            if (shader == null)
            {
                Debug.LogError("Shader 'UI/GradientMaskedElement' not found!");
                return baseMaterial;
            }
        ...
        // (Например, от RoundedImageV2 или стандартной Mask).
